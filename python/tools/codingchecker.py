...
class CodingChecker(object):
    ...
    def __init__(self, checked_file=None):
        ...
        if os.path.exists(self.checked_file):
            if os.path.isdir(self.checked_file):
                ...
                self.tempdir = os.path.join(self.checked_file, datetime.now().strftime('%Y%m%d%H%M%S%f'))
            else :
                self.tempdir = os.path.join(os.path.abspath(os.path.dirname(self.checked_file)), datetime.now().strftime('%Y%m%d%H%M%S%f'))
    ...
    def _is_need_checked(self, fl):
        return os.path.splitext(fl)[1].lower() in CodingChecker.CHECKED_FILE_EXT
    ...
    def _check_single_file(self, fl):
        ...
        if encoding['encoding'] != 'utf-8':
            self.logger.write('%s\t%s\t%f;' % (fl,encoding['encoding'],encoding['confidence']))
